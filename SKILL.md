...
- figure: `{type:figure, ref:"images/...", caption:"...", fig_number:"图X-Y or Figure X.Y", context:"What example/section it belongs to"}`
...
11. **图片引用检测 (Image Reference Detection)**: 当源文本中出现图片引用时（如图X-Y、Figure X.Y、图 X-Y(a) 等），必须提取为 figure block。记录图号（fig_number）、所属上下文（context: 属于哪个例题/概念/定律）和标题（caption）。中文教材特别注意 "如图X-Y所示" 这类行内引用。
12. **例题配图**: 例题（example）依赖的图片必须与例题一起编号。若例题描述以"如图X-Y所示"开头，figure 和 example 成对出现。
...
- **physics**: Identify physical laws with names. Extract equations with variable definitions. Capture experimental setups with procedures. Note physical constants with values and units. **Detect figure references (图X-Y, Figure X.Y) — especially those tied to examples and laws. Capture figure numbers and their context.**
...
- **engineering**: Extract design equations with parameter definitions. Capture engineering standards and specifications. Note safety factors and design margins. **Detect figure/diagram references (图X-Y) tied to design examples.**
...
**figure**: 根据教材语言自动选择命名约定：
- 英文教材: `![Figure X.Y：{caption}](images/ch{chapter:02d}-{slug}.png)`
- 中文教材: `![图X-Y](images/FigX-Y.png)` 或 `![图X-Y：{caption}](images/FigX-Y.png)`
- 若图片文件实际上不存在于 images/ 目录，仍然生成正确的 Markdown 图片引用（用户可后续补图）。
- 图片应放置在引用它的段落之后、下一段之前，保持与原文相同的逻辑位置。
- **若源文本来自含图片的 Word 文档或 PDF**，提醒用户可从源文档提取图片放入 `images/` 目录。
...
### G. Figures & Images
- [ ] Every figure reference (图X-Y / Figure X.Y) in source text has a corresponding Markdown image link
- [ ] Image paths correctly point to `images/` directory
- [ ] Image placeholders use consistent naming convention (FigX-Y or chXX-slug)
- [ ] Figures are placed after their first mention paragraph
- [ ] No broken image references (file path exists or intentional placeholder)
...
- **physics**: Verify equation variable consistency. Check unit consistency across derivations. Validate constant values/units. Verify vector/scalar notation. **Verify all 图X-Y references have corresponding image placeholders.**
...
## Image Handling — 图片提取与嵌入工作流

### 场景

当教材源为 Word 文档（.docx）或 PDF 且包含图片时，笔记中的 `图X-Y` 引用需要配实际图片。
...
```bash
# 扫描所有笔记中的图号引用
grep -oP '图\s*\d+[-–—]\d+' Ch*.md | sort -u
```

**Step 2 — 从 Word 文档提取图片**:
```python
from docx import Document
doc = Document('source.docx')
# 通过 paragraph 文本匹配图号，找到最近 image rId
# 提取 blob 并保存到 images/FigX-Y.png
```

**Step 3 — 图片命名约定**:
- `images/Fig{chapter}-{number}.{ext}` — 例: `images/Fig2-7.png`
- 或 `images/ch{chapter:02d}-{slug}.{ext}` — 例: `images/ch02-kinematics.png`
- 图片格式: 自动检测 blob header（`\x89PNG` → .png, `\xFF\xD8` → .jpeg）

**Step 4 — 插入 Markdown**:
- 在笔记中首次提及该图后的空行处插入 `![图X-Y](images/FigX-Y.png)`
- 图片文件缺失时不报错 — Markdown 渲染器会显示 alt text 占位

### 常见问题

| 问题 | 解决方案 |
|------|---------|
| 图片太小（< 2KB） | 可能是 OCR artifact，删掉取同一图号的大版本 |
| 图号附近无 image | 扩大搜索 offset 到 ±20 paragraphs |
| 多个图号匹配同一段落 | 取最近且最大的图片 |
| 源文档无图片层 | 使用原 PDF + PyMuPDF 渲染页面截图 |
